...
//SD初始化完成后采用12MHz传输速度，24MHz传输会导致数据读写异常
//使用DMA模式的时候,传输速率可以到24Mhz,不过如果你的卡不是高速卡,可能也会出错
//出错就请降低时钟,使用查询模式的话,推荐SDIO_TRANSFER_CLK_DIV设置为3或者更大
#define SDIO_INIT_CLK_DIV                  ((uint8_t)0xB2)      //SDIO初始化频率，最大400Kh  
//#define SDIO_TRANSFER_CLK_DIV              ((uint8_t)0x1)       //SDIO传输频率,该值太小可能会导致读写文件出错 
#define SDIO_TRANSFER_CLK_DIV              ((uint8_t)0x04)       //SDIO传输频率,该值太小可能会导致读写文件出错 
...
#define SD_INTERRUPT_MODE               ((uint32_t)0x00000002)
...
extern SDCardInfo_Type  SDCardInfo;
